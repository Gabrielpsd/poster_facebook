...
session_start();
    ...
    use Aws\S3\S3Client;
    print_r('Publicando no Instagram');
            ...
            try{
                ...
                if(!empty($resultArray['ObjectURL']))
                {
                    ...
                    while($Container < 2)
                    {
                        ...
                        $Json = json_encode(['image_url'=>$s3FileLink]);
                        ...
                        {   
                            ...
                            while($published || ($tentativa < 2))
                            {
                                ...
                                $ch = curl_init();
                                curl_setopt($ch, CURLOPT_URL,"https://graph.instagram.com/".GRAPH_VERSION."/".$_SESSION['ig_user_id']."/media_publish?access_token=".$_SESSION['ig_acess_token']."&creation_id=".$response->id);
                                curl_setopt($ch, CURLOPT_POST, true);
